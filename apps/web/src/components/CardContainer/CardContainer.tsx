...
export default function CardContainer({ id, state, dispatch }: CardContainerProps) {
    ...
    const displayCards =
        draggingId == null ? cards : cards.filter((c) => c !== draggingId);

    const shouldShowGhost =
        draggingId != null && state.userActions.mouseHoveringContainer === id;
    ...
    return (
        ...
        >
            {displayCards.map((cardId, i) => {
                ...
                return (
                    <React.Fragment key={`row-${cardId}`}>
                        {ghostHere && <CardGhost key={`ghost-${i}`} />}
                        <Card
                            id={cardId}
                            state={state}
                            dispatch={dispatch}
            ...
            {shouldShowGhost && ghostIndex === displayCards.length && (
                <CardGhost key="ghost-end" />
            )}
