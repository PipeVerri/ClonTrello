import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faSquarePlus } from "@fortawesome/free-solid-svg-icons";
import {BoardAction, BoardState, CardInfo} from "./reducer";
import {Dispatch} from "react";
import Card from "../Card/Card";
import {Editing} from "../../app/page";
import {Setter} from "../../utils/types";

interface CardContainerProps {
    id: number,
    state: BoardState,
    dispatch: Dispatch<BoardAction>,
    editing: Editing,
    setEditing: Setter<Editing>
}

export default function CardContainer({ id, state, dispatch, editing, setEditing }: CardContainerProps) {
    function createCard() {
        const defaultCard: CardInfo = {
            title: ""
        }
        dispatch({type: "addCard", containerId: id, cardInfo: defaultCard})
    }

    const handleMouseEnter = () => {
        setEditing(oldVal => {
            return {...oldVal, mouseHoveringContainer: id}
        })
    }
    const handleMouseLeave = () => {
        setEditing(oldVal => {
            return {...oldVal, mouseHoveringContainer: null}
        })
    }

    return (
        <div className="flex flex-col bg-amber-200 w-[300px] gap-2 p-4 rounded-lg drop-shadow-lg" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            {state.containerCards[id].map((id, _) => (
                (editing.dragging === id) ? null : <Card key={id} id={id} state={state} dispatch={dispatch} setEditing={setEditing} />
            ))}
            <button className="bg-green-500 text-center text-lg py-2 font-semibold text-white rounded-lg shadow-lg" onClick={createCard}>
                <FontAwesomeIcon icon={faSquarePlus} size="lg" />
            </button>
        </div>
    )
}